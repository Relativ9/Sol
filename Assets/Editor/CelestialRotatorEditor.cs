...
namespace Sol.Editor
{
    ...
    public class CelestialRotatorEditor : UnityEditor.Editor
    {
        ...
        private void DrawHeader()
        {
            ...
            EditorGUILayout.HelpBox("Controls the rotation of celestial bodies (suns, moons) based on time progression and seasonal data.", MessageType.Info);
        ...
        private void DrawCelestialBodyConfiguration()
        {
            ...
            EditorGUILayout.PropertyField(isMoonProp, new GUIContent("Is Moon", 
                "Whether this is a moon (affects orbital calculation, including a monthly drift) or a star/sun."));

            EditorGUILayout.PropertyField(baseRotationOffsetProp, new GUIContent("Base Rotation Offset", 
                "Additional rotation offset applied to the calculated celestial position. Use to make sure bodies rise and set at different points on the horizon"));
        ...
        private void DrawRotationMethod()
        {
            ...
            EditorGUILayout.PropertyField(smoothRotationProp, new GUIContent("Smooth Rotation", 
                "Enable smooth interpolation between rotation updates. Provides smoother visual movement but uses more CPU, overrides the optimization setting. Not noticable for most orbital and day/night values (only use for extremely quick day/night cycles or fast moving sattalites)."));
            ...
            EditorGUILayout.Space(5);
            string configInfo = GetRotationConfigInfo();
        ...
        private void DrawDebugSettings()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                {
                    CelestialRotator rotator = (CelestialRotator)target;
                    Debug.Log($"[CelestialRotator] {rotator.name}: Rotation={rotator.CurrentRotation}, Quaternion={rotator.CurrentQuaternion}");
                }
            }
        ...
        private string GetRotationConfigInfo()
        {
            ...
            if (smoothRotation && optimization)
            {
                info = "Configuration: Pure quaternion rotation with smooth interpolation and performance optimization.\n";
                info += "• Rotation calculated every frame for smooth time progression\n";
                info += "• Smooth interpolation applied every frame\n";
                info += "• No gimbal lock issues at any angle\n";
                info += "• Moderate CPU usage";
            }
            else if (smoothRotation && !optimization)
            {
                info = "Configuration: Pure quaternion rotation with smooth interpolation, no optimization.\n";
            ...
            {
                info = "Configuration: Pure quaternion rotation with optimization, no smoothing.\n";
            ...
            {
                info = "Configuration: Standard pure quaternion rotation.\n";
                info += "• No gimbal lock issues at any angle\n";
                info += "• Direct rotation updates every frame";
            }
