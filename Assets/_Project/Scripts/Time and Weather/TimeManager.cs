...
//         private int targetSeasonIndex = -1; // -1 means no transition
...
//
//         /// <summary>
//         /// Currently active season index based on day of year
//         /// </summary>
//         public int CurrentSeasonIndex => currentSeasonIndex;
//         
//         /// <summary>
//         /// Currently active season name
//         /// </summary>
...
//
//         /// <summary>
//         /// Target season index when transitioning (-1 if no transition)
//         /// </summary>
//         public int TargetSeasonIndex => targetSeasonIndex;
//         
//         /// <summary>
//         /// Target season name when transitioning
//         /// </summary>
...
//         /// </summary>
//         public event System.Action<int, string> OnSeasonChanged; // (seasonIndex, seasonName)
//
//         /// <summary>
...
//             }
//
//             // Validate flexible season configuration
...
// //     if (worldTimeData.GetSeasonCount() == 0)
// //     {
// //         Debug.LogWarning("[TimeManager] No seasons configured! Use 'Create Default Sol Seasons' context menu on WorldTimeData to set up standard seasons.");
// //     }
// //     else
// //     {
// //         // Check if all seasons have seasonal data assigned
...
//         //     if (worldTimeData == null) return;
//         //
//         //     // Use the new flexible season system
...
//         //     {
//         //         // Update season range even if season hasn't changed (for progress calculations)
//         //         currentSeasonRange = worldTimeData.GetSeasonRange(newSeasonIndex);
//         //     }
//         //
//         //     // Update transition state
//         //     UpdateSeasonTransition();
...
//             if (worldTimeData == null) return;
//
//             // Use the flexible season system
//             int newSeasonIndex = worldTimeData.GetSeasonIndexForDay(currentDay);
...
//             else
//             {
//                 // Update season range even if season hasn't changed
...
//         {
//             int previousSeasonIndex = currentSeasonIndex;
//             string previousSeasonName = currentSeasonName;
//     
...
//             if (enableDebugLogging)
//             {
//                 Debug.Log($"[TimeManager] Season Changed: {previousSeasonName} → {currentSeasonName}");
//             }
//         }
...
//         /// Handles both regular season transitions and flexible season system.
//         /// </summary>
...
//                     
//                     // Get next season (with wraparound)
...
//             }
//
//             // Handle year boundary transition
...
// //     seasonTransitionProgress = 0f;
// //
// //     // Get current season index and total seasons
// //     int currentSeasonIndex = worldTimeData.GetSeasonIndexForDay(currentDay);
// //     int totalSeasons = worldTimeData.GetSeasonCount();
// //     
// //     if (totalSeasons == 0) return;
// //
...
// //             // Get next season (with wraparound)
// //             int nextSeasonIndex = (seasonRange.seasonIndex + 1) % totalSeasons;
// //             var nextSeasonConfig = worldTimeData.GetSeasonConfiguration(nextSeasonIndex);
// //             targetSeason = nextSeasonConfig?.solSeasonType ?? Season.Gro;
// //             
// //             seasonTransitionProgress = 0.5f + (0.5f * (halfTransition - daysFromSeasonEnd) / halfTransition);
// //             
// //             LogTransitionState("End of season transition");
...
// //             // We're in the first half of transition (coming from previous season)
// //             isInSeasonTransition = true;
// //             targetSeason = seasonRange.solSeasonType; // Current season is the target
// //             seasonTransitionProgress = 0.5f * (daysFromSeasonStart / halfTransition);
// //             
// //             LogTransitionState("Start of season transition");
// //             return;
...
// //     }
// //
// //     // Handle year boundary transition (last season -> first season)
// //     HandleYearBoundaryTransition(halfTransition, totalSeasons);
// // }
//
//         // /// <summary>
...
//         
//         /// <summary>
//         /// Handles season transitions that occur across year boundaries.
//         /// </summary>
//         /// <param name="halfTransition">Half of the total transition duration in days</param>
//         /// <param name="totalSeasons">Total number of seasons in the system</param>
...
//     
//             // Get the last season in the year
//             var lastSeason = seasonRanges[seasonRanges.Length - 1];
//             var firstSeason = seasonRanges[0];
//     
//             // Check if we're in the last season approaching year end
...
//                 {
//                     // Transitioning from last season to first season of next year
//                     isInSeasonTransition = true;
//                     targetSeasonIndex = 0; // First season index
//                     targetSeasonName = firstSeason.seasonName;
//                     seasonTransitionProgress = 0.5f + (0.5f * (halfTransition - daysFromYearEnd) / halfTransition);
//             
//                     LogTransitionState($"Year boundary transition ({lastSeason.seasonName} → {firstSeason.seasonName})");
//                     return;
//                 }
//             }
//
//             // Check if we're in the first season coming from previous year
//             if (currentDay <= firstSeason.endDay && currentDay <= halfTransition)
//             {
//                 // Coming from last season into first season
//                 isInSeasonTransition = true;
//                 targetSeasonIndex = 0;
//                 targetSeasonName = firstSeason.seasonName;
//                 seasonTransitionProgress = 0.5f * (currentDay / halfTransition);
//         
//                 LogTransitionState($"Year boundary transition completion ({lastSeason.seasonName} → {firstSeason.seasonName})");
//                 return;
//             }
//         }
//         // private void HandleYearBoundaryTransition(int halfTransition, int totalSeasons)
...
//         //             // Transitioning from last season to first season of next year
//         //             isInSeasonTransition = true;
//         //             targetSeason = firstSeason.solSeasonType;
//         //             seasonTransitionProgress = 0.5f + (0.5f * (halfTransition - daysFromYearEnd) / halfTransition);
//         //     
//         //             LogTransitionState($"Year boundary transition ({lastSeason.seasonName} → {firstSeason.seasonName})");
//         //             return;
...
//         
//         // /// <summary>
//         // /// Gets the next season in the seasonal cycle using flexible system.
//         // /// </summary>
//         // /// <param name="season">Current season</param>
//         // /// <returns>Next season in the cycle</returns>
//         // private Season GetNextSeason(Season season)
//         // {
//         //     if (worldTimeData == null) return Season.Gro;
//         //
//         //     var seasonRanges = worldTimeData.SeasonRanges;
//         //     if (seasonRanges.Length == 0) return Season.Gro;
//         //
//         //     // Find current season index
//         //     for (int i = 0; i < seasonRanges.Length; i++)
//         //     {
//         //         if (seasonRanges[i].solSeasonType == season)
//         //         {
//         //             // Return next season (with wraparound)
//         //             int nextIndex = (i + 1) % seasonRanges.Length;
//         //             return seasonRanges[nextIndex].solSeasonType;
//         //         }
//         //     }
//         //
//         //     // Fallback to first season
//         //     return seasonRanges[0].solSeasonType;
//         // }
//
//         /// <summary>
...
//             {
//                 Debug.Log($"[TimeManager] {transitionType}: {currentSeasonName} → {targetSeasonName}, Progress: {seasonTransitionProgress:P1}");
//             }
//         }
//         // private void LogTransitionState(string transitionType)
//         // {
...
//
//                 // Calculate total game time from existing values
...
//
//         /// <summary>
//         /// Gets the seasonal data configuration for a specific season index.
//         /// </summary>
...
//         
//         /// <summary>
//         /// Determines which season index contains the specified day of year.
//         /// </summary>
...
//                 ? $"\nTransition: {currentSeasonName} → {targetSeasonName} (Progress: {seasonTransitionProgress:P1})"
//                 : "\nTransition: None";
//
...
//             if (worldTimeData != null && worldTimeData.MonthsPerYear > 0)
//             {
//                 calendarInfo = $"\nCalendar: {currentMonth.name} {currentDayOfMonth}";
//             }
//         
...
//                       $"\nSeason: {currentSeasonName}" + seasonInfo +
...
//
//             int totalDays = worldTimeData.GetTotalSeasonDays();
//             string totalDaysStr = totalDays > 0 ? totalDays.ToString() : worldTimeData.totalDaysInYear.ToString();
//
//             return $"Time: {CurrentTimeDisplay}" + calendarInfo +
//                    $"\nDay {currentDay} of {totalDaysStr}, Year {currentYear}\n" +
//                    $"Season: {currentSeasonName} ({SeasonProgress:P0} complete)\n" +
//                    $"Time of Day: {currentTimeOfDay}\n" +
//                    $"Time Scale: {timeScale:F1}x" +
//                    (isInSeasonTransition ? $"\nTransitioning to {targetSeasonName} ({seasonTransitionProgress:P0})" : "");
//         }
//
//         // /// <summary>
...
//     
//             int totalDays = worldTimeData.GetTotalSeasonDays();
//             if (totalDays > 0 && targetDayOfYear > totalDays)
//             {
//                 targetDayOfYear = totalDays;
//             }
//
...
//         
//                 Debug.Log($"Month {monthIndex}: {GetFormattedDate()} (Day {currentDay}, Season: {currentSeasonName})");
//             }
//     
...
//             if (worldTimeData != null && worldTimeData.MonthsPerYear > 0)
//             {
//                 debugCalendarDate = $"{currentMonth.name} {currentDayOfMonth}";
//             }
//             else
//             {
...
//             }
//     
//             debugSeasonInfo = $"{currentSeasonName} - {SeasonProgress:P1} complete ({DaysRemainingInSeason} days remaining)";
//             debugTransitionInfo = isInSeasonTransition 
//                 ? $"Transitioning to {targetSeasonName} ({seasonTransitionProgress:P1})"
